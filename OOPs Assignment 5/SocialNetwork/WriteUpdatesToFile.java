	...
	public void writeEntityDetailsToFile(Graph<Entity> graph,
		...
		for (String id : entityId) {
			if ("user".equalsIgnoreCase(graph.getNodeById(id).getType())) {
				...
				userDetails += user1.getId() + "," + user1.getName() + ","+ user1.getPassword() + "," + user1.getContact() + ","+ user1.getAge() + "," + user1.getType() + "\n";
			} else {
				Organization organisation = (Organization) graph
						.getNodeById(id);
				userDetails += organisation.getId() + ","+ organisation.getName() + ","+ organisation.getPassword() + ","+ organisation.getContact() + ","+ organisation.getAddress() + ","+ organisation.getType() + "\n";
